...
class CollectionViewModel : ViewModel() {
    ...
    fun getAllCollectionsWithUsernames(
        ...
        viewModelScope.launch {
            try {
                val collectionDocs = db.collection("collections").get().await()

                val filteredDocs = collectionDocs.documents.filter {
                    it.getString("iduser") != currentUserId
                }

                val collections = mutableListOf<UserCollection>()

                for (doc in filteredDocs) {
                    val base = doc.toObject(UserCollection::class.java)?.copy(id = doc.id)
                    val iduser = base?.iduser ?: continue

                    val userDoc = db.collection("users").document(iduser).get().await()
                    val username = userDoc.getString("username") ?: "Sconosciuto"

                    val enriched = base.copy(username = username)
                    collections.add(enriched)
                }

                onSuccess(collections)
            } catch (e: Exception) {
                onFailure("Errore nel recupero collezioni con username: ${e.message}")
            }
        }
    }
    ...
    fun getUsernameById(userId: String, onResult: (String) -> Unit) {
        FirebaseFirestore.getInstance()
            ...
            .addOnSuccessListener { document ->
                val username = document.getString("username") ?: "Utente sconosciuto"
                onResult(username)
            }
            .addOnFailureListener {
                onResult("Errore nel recupero")
            }
    }

    fun getProfileImageById(userId: String, onResult: (String?) -> Unit) {
        FirebaseFirestore.getInstance()
            .collection("users")
            .document(userId)
            .get()
    ...
    fun getCollectionsByUserId(userId: String, onResult: (List<UserCollection>) -> Unit) {
        FirebaseFirestore.getInstance()
